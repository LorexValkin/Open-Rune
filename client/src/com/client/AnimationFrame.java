...
public final class AnimationFrame {
	...
	public static void method528() {
	 	animationlist = new AnimationFrame[4500][0];
	}

	public static void load(int file, byte[] fileData){
		try {
			Buffer stream = new Buffer(fileData);
            AnimationSkeleton AnimationSkeleton = new AnimationSkeleton(stream);
			int k1 = stream.readUnsignedWord();
			animationlist[file] = new AnimationFrame[(int)(k1*3)];
			int ai[] = new int[500];
			int ai1[] = new int[500];
			int ai2[] = new int[500];
			int ai3[] = new int[500];
			for(int l1 = 0; l1 < k1; l1++) {
				...
				AnimationFrame class36 = animationlist[file][i2] = new AnimationFrame();
				class36.skeleton = AnimationSkeleton;
				...
				for(int i3 = 0; i3 < j2; i3++) {
					...
					if(j3 > 0) {
						if(AnimationSkeleton.boneTypes[i3] != 0) {
							for(int l3 = i3 - 1; l3 > k2; l3--) {
								if(AnimationSkeleton.boneTypes[l3] != 0)
									continue;
								ai[l2] = l3;
								ai1[l2] = 0;
								ai2[l2] = 0;
						...
						ai[l2] = i3;
						short c = 0;
						if(AnimationSkeleton.boneTypes[i3] == 3)
							c = (short)128;
						if((j3 & 1) != 0)
	...
	public static AnimationFrame forId(int i) {
		try {
			String s = "";
			int file = 0;
			int k = 0;
			...
			if(animationlist[file].length == 0) {
				clientInstance.onDemandFetcher.requestFile(1, file);
				return null;
			}
	...
	public static AnimationFrame animationlist[][];
