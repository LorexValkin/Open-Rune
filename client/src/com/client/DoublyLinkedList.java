package com.client;
// Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) 

public final class DoublyLinkedList {

	public DoublyLinkedList() {
		head = new CacheableNode();
		head.prev = head;
		head.next = head;
	}

	public void insertHead(CacheableNode node) {
		if (node.next != null)
			node.unlink();
		node.next = head.next;
		node.prev = head;
		node.next.prev = node;
		node.prev.next = node;
	}

	public void insertTail(CacheableNode node) {
		if (node.next != null)
			node.unlink();
		node.next = head;
		node.prev = head.prev;
		node.next.prev = node;
		node.prev.next = node;
	}

	public final void method894(CacheableNode class3, CacheableNode class3_27_) {
		if (class3.prev != null) {
			class3.unlink();
		}

		class3.next = class3_27_;
		class3.prev = class3_27_.prev;
		class3.prev.next = class3;
		class3.next.prev = class3;
	}

	
	public CacheableNode popHead() {
		CacheableNode node = head.prev;
		if (node == head) {
			return null;
		} else {
			node.unlink();
			return node;
		}
	}

	public CacheableNode reverseGetFirst() {
		CacheableNode node = head.prev;
		if (node == head) {
			current = null;
			return null;
		} else {
			current = node.prev;
			return node;
		}
	}

	public CacheableNode getFirst() {
		CacheableNode node = head.next;
		if (node == head) {
			current = null;
			return null;
		} else {
			current = node.next;
			return node;
		}
	}

	public CacheableNode reverseGetNext() {
		CacheableNode node = current;
		if (node == head) {
			current = null;
			return null;
		} else {
			current = node.prev;
			return node;
		}
	}

	public CacheableNode getNext() {
		CacheableNode node = current;
		if (node == head) {
			current = null;
			return null;
		}
		current = node.next;
		return node;
	}

	public void removeAll() {
		if (head.prev == head)
			return;
		do {
			CacheableNode node = head.prev;
			if (node == head)
				return;
			node.unlink();
		} while (true);
	}

	public final CacheableNode head;
	private CacheableNode current;
}
