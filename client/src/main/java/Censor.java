...
final class Censor {
	...
	private static void readBadEnc(Stream stream)
	{
		...
		readBadEnc(stream, aCharArrayArray621, aByteArrayArrayArray622);
	}
	...
	private static void readDomainEnc(Stream stream)
	{
		...
		aCharArrayArray623 = new char[i][];
			readTldList(aCharArrayArray623, stream);
	}
	...
	private static void readBadEnc(Stream stream, char ac[][], byte abyte0[][][])
	{
	...
	private static void readTldList(char ac[][], Stream stream)
	{
		for(int j = 0; j < ac.length; j++)
		{
			char ac1[] = new char[stream.readUnsignedByte()];
	...
	private static void stripInvalid(char ac[])
	{
		int i = 0;
		for(int j = 0; j < ac.length; j++)
		{
	...
	public static String doCensor(String s)
	{
		...
		applyFilter(s1.toCharArray(), ac);
		filterBadWords(ac);
		long l1 = System.currentTimeMillis();
		return (new String(ac)).trim(); //xxx chat filter, return s to avoid
	}

	private static void applyFilter(char ac[], char ac1[])
	{
		for(int j = 0; j < ac.length; j++)
	...
	private static void filterBadWords(char ac[])
	{
		boolean flag = true;
		for(int j = 0; j < ac.length; j++)
		{
	...
	private static void filterDomains(char ac[])
	{
		for(int i = 0; i < 2; i++)
		{
			for(int j = aCharArrayArray621.length - 1; j >= 0; j--)
				matchComboList(aByteArrayArrayArray622[j], ac, aCharArrayArray621[j]);

		}
	}

	private static void filterFragments(char ac[])
	{
		char ac1[] = ac.clone();
		char ac2[] = {
			'(', 'a', ')'
		};
		matchComboList(null, ac1, ac2);
		char ac3[] = ac.clone();
		char ac4[] = {
			'd', 'o', 't'
		};
		matchComboList(null, ac3, ac4);
		for(int i = aCharArrayArray623.length - 1; i >= 0; i--)
			filterTldWord(ac, aCharArrayArray623[i], ac3, ac1);
	}

	private static void filterTldWord(char ac[], char ac1[], char ac2[], char ac3[])
	{
		...
		for(int k = 0; k <= ac.length - ac1.length; k += j)
		{
			...
			while(l < ac.length) 
			{
				...
				if(l + 1 < ac.length)
					c1 = ac[l + 1];
				if(i1 < ac1.length && (j1 = charCombinationA(c, ac1[i1], c1)) > 0)
				{
					l += j1;
					i1++;
					continue;
				...
				if(i1 == 0)
					break;
				if((j1 = charCombinationA(c, ac1[i1 - 1], c1)) > 0)
				{
					l += j1;
					if(i1 == 1)
						j++;
				...
				if(i1 >= ac1.length || !isUppercase(c))
					break;
				l++;
			}
			if(i1 >= ac1.length)
			{
				boolean flag1 = false;
				int k1 = findBadWordIndex(ac, ac3, k);
				int l1 = findBadFragment(ac2, l - 1, ac);
				if(k1 > 2 || l1 > 2)
					flag1 = true;
				if(flag1)
				{
	...
	private static int findBadWordIndex(char ac[], char ac1[], int j)
	{
		if(j == 0)
			return 2;
		for(int k = j - 1; k >= 0; k--)
		...
		if(l >= 3)
			return 4;
		return !isUppercase(ac[j - 1]) ? 0 : 1;
	}

	private static int findBadFragment(char ac[], int i, char ac1[])
	{
		if(i + 1 == ac1.length)
			return 2;
		for(int j = i + 1; j < ac1.length; j++)
		{
			if(!isUppercase(ac1[j]))
				break;
			if(ac1[j] == '.' || ac1[j] == ',')
				return 3;
		}
		...
		if(k >= 3)
			return 4;
		return !isUppercase(ac1[i + 1]) ? 0 : 1;
	}

	private static void filterNumericDomain(char ac[])
	{
		char ac1[] = ac.clone();
		char ac2[] = {
			'd', 'o', 't'
		...
		for(int i = 0; i < aCharArrayArray624.length; i++)
			filterDomainDot(ac3, aCharArrayArray624[i], anIntArray625[i], ac1, ac);

	}

	private static void filterDomainDot(char ac[], char ac1[], int i, char ac2[], char ac3[])
	{
		...
		for(int k = 0; k <= ac3.length - ac1.length; k += j)
		{
			...
			{
				boolean flag1 = false;
				int k1 = findNumericDomain(ac3, k, ac2);
				int l1 = findDotDomain(ac3, ac, l - 1);
				if(i == 1 && k1 > 0 && l1 > 0)
					flag1 = true;
	...
	private static int findNumericDomain(char ac[], int j, char ac1[])
	{
		if(j == 0)
			return 2;
		for(int k = j - 1; k >= 0; k--)
	...
	private static int findDotDomain(char ac[], char ac1[], int i)
	{
		if(i + 1 == ac.length)
			return 2;
		...
		if(k >= 5)
			return 4;
		return !isUppercase(ac[i + 1]) ? 0 : 1;
	}

	private static void matchComboList(byte abyte0[][], char ac[], char ac1[])
	{
		if(ac1.length > ac.length)
			return;
		boolean flag = true;
		...
		for(int k = 0; k <= ac.length - ac1.length; k += j)
		{
			...
			while(l < ac.length && (!flag2 || !flag3)) 
			{
				...
				if(l + 1 < ac.length)
					c2 = ac[l + 1];
				if(i1 < ac1.length && (k1 = charCombinationB(c2, c, ac1[i1])) > 0)
				{
					if(k1 == 1 && isDigit(c))
						flag2 = true;
				...
				if(i1 == 0)
					break;
				if((k1 = charCombinationB(c2, c, ac1[i1 - 1])) > 0)
				{
					l += k1;
					if(i1 == 1)
						j++;
			...
			if(i1 >= ac1.length && (!flag2 || !flag3))
			{
				...
				if(!flag1)
				{
					...
					if(l < ac.length)
						c3 = ac[l];
					byte byte0 = getCharCategory(c1);
					byte byte1 = getCharCategory(c3);
					if(abyte0 != null && matchComboEntry(byte0, abyte0, byte1))
						flag4 = false;
				} else
				{
					boolean flag5 = false;
					boolean flag6 = false;
					if(k - 1 < 0 || isUppercase(ac[k - 1]) && ac[k - 1] != '\'')
						flag5 = true;
					if(l >= ac.length || isUppercase(ac[l]) && ac[l] != '\'')
						flag6 = true;
					if(!flag5 || !flag6)
					{
						boolean flag7 = false;
						...
						if(flag5)
							k2 = k;
						for(; !flag7 && k2 < l; k2++)
							if(k2 >= 0 && (!isUppercase(ac[k2]) || ac[k2] == '\''))
							{
								char ac2[] = new char[3];
								int j3;
								for(j3 = 0; j3 < 3; j3++)
								{
									if(k2 + j3 >= ac.length || isUppercase(ac[k2 + j3]) && ac[k2 + j3] != '\'')
										break;
									ac2[j3] = ac[k2 + j3];
								}
								...
								if(j3 == 0)
									flag8 = false;
								if(j3 < 3 && k2 - 1 >= 0 && (!isUppercase(ac[k2 - 1]) || ac[k2 - 1] == '\''))
									flag8 = false;
								if(flag8 && !containsDigits(ac2))
									flag7 = true;
							}
	...
	private static boolean matchComboEntry(byte byte0, byte abyte0[][], byte byte2)
	{
		int i = 0;
	...
	private static int charCombinationA(char c, char c1, char c2)
	{
		if(c1 == c)
			return 1;
		if(c1 == 'o' && c == '0')
	...
	private static int charCombinationB(char c, char c1, char c2)
	{
		if(c2 == c1)
			return 1;
		if(c2 >= 'a' && c2 <= 'm')
	...
	private static byte getCharCategory(char c)
	{
		if(c >= 'a' && c <= 'z')
			return (byte)((c - 97) + 1);
		if(c == '\'')
	...
	private static void capitalizeFirst(char ac[])
	{
		int j;
		int k = 0;
		int l = 0;
		int i1 = 0;
		while((j = indexOfDigit(ac, k)) != -1)
		{
			boolean flag = false;
			for(int j1 = k; j1 >= 0 && j1 < j && !flag; j1++)
				if(!isUppercase(ac[j1]) && !isLetter(ac[j1]))
					flag = true;
	...
	private static int indexOfDigit(char ac[], int i)
	{
	...
	private static int indexOfLetter(char ac[], int j)
	{
	...
	private static boolean containsDigits(char ac[])
	{
		boolean flag = true;
	...
	private static int countUppercase(char ac[])
	{
		if(ac.length > 6)
			return 0;
		int k = 0;
