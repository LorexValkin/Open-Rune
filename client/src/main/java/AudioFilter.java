...
final class AudioFilter
{
	...
	private float getFilterGain(int i, int j, float f)
	{
	...
	public void decodeFilter(Stream stream, SoundEnvelope soundEnvelope)
	{
		...
		if(i != 0)
		{
			...
			if(j != 0 || anIntArray668[1] != anIntArray668[0])
				soundEnvelope.decodeShape(stream);
		} else
		{
